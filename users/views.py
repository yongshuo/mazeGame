...
def register_action(request):
    ...
    if not full_name:
        request.session['REGISTER_ERROR'] = ugettext_lazy('Please provide full name !').encode('utf-8')
        return redirect('/register/')
    ...
    try:
        ...
        try:
            entity_login = EntityLogin(full_name = full_name, email = email, search_text = email, password = password)
            entity_login.save()
        except Exception as e:
...
def login_action(request):
    ...
    try:
        ...
        if entity_login.verify_password(password) == False:
            ...
            request.session['NAME'] = entity_login.full_name
...
def account(request):
    ...
    context['NAME'] = request.session['NAME'] if request.session.has_key('NAME') else None
